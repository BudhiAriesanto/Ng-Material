 ...
 *
 */
export class MdCompilerProvider {
  private respectPreAssignBindingsEnabled_: boolean = false;
  constructor(protected $compileProvider?: ng.ICompileProvider) {
    'ngInject';
  }
  /**
   * @ngdoc method
   ...
   */
  @method
  public respectPreAssignBindingsEnabled(respected: boolean) {
    if (ng.isDefined(respected)) {
      this.respectPreAssignBindingsEnabled_ = respected;
      return this;
    }
    return this.respectPreAssignBindingsEnabled_;
  }
  /**
   * @private
   * @description
   ...
   */
  @method
  public getPreAssignBindingsEnabled() {
    if (!this.respectPreAssignBindingsEnabled_) {
    ...
    if (typeof this.$compileProvider.preAssignBindingsEnabled === 'function') {
      return this.$compileProvider.preAssignBindingsEnabled();
    }

    // Flag respected but not present => apply logic based on AngularJS version used.
    if (ng.version.major === 1 && ng.version.minor < 6) {
      // AngularJS <1.5.10
      return true;
    }
 ...
 *
 */
@NgProvider(MdCompilerProvider)
export class MdCompilerServices extends MdCompilerProvider {
  constructor(private $q: ng.IQService, private $templateRequest: ng.ITemplateRequestService, private $injector: ng.auto.IInjectorService, private $compile: ng.ICompileService, private $controller: any) {
    'ngInject';
    super();
  }
  /**
   ...
   */
  private fetchContentElement(options: any): {element: JQLite, restore: Function} {

    let contentEl = options.contentElement;
    let restoreFn = null;

    if (ng.isString(contentEl)) {
    ...
    return {
      element: ng.element(contentEl),
      restore: restoreFn
    };

    function createRestoreFn(element: HTMLElement) {
   ...
   */
  private prepareContentElement(options: any): ng.IPromise<any> {

    let contentElement = this.fetchContentElement(options);
   ...
   */
  private createController(options: any, injectLocals: any, locals: any): ng.IController {
    let ctrl: ng.IController;
    let preAssignBindingsEnabled = this.getPreAssignBindingsEnabled();
    ...
    if (preAssignBindingsEnabled) {
      let invokeCtrl = this.$controller(options.controller, injectLocals, true);

      if (options.bindToController) {
        Object.assign(invokeCtrl.instance, locals);
      }
   ...
   */
  private compileElement(locals: any, element: JQLite, options: any): ICompileData {
    let self = this;
    let ngLinkFn = this.$compile(element);

    let compileData: ICompileData = {
      element: element,
    ...
    function linkFn(scope: any) {
      ...
      if (options.controller) {
        ...
        let injectLocals = Object.assign({}, locals, {
          $element: element
        });

        // Create the specified controller instance.
        let ctrl = self.createController(options, injectLocals, locals);
   ...
   * @returns {!Object} Compile data with link function.
   */
  private compileTemplate(options: any) {

    let self = this;
    let templateUrl = options.templateUrl;
    let template = options.template || '';
    let resolve = Object.assign({}, options.resolve);  // angular.extend({}, options.resolve);
    let locals = Object.assign({}, options.locals);
    let transformTemplate = options.transformTemplate || ng.identity;

    // Take resolve values and invoke them.
    // Resolves can either be a string (value: 'MyRegisteredAngularConst'),
    // or an invokable 'factory' of sorts: (value: function ValueGetter($dependency) {})
    ng.forEach(resolve, function(value: any, key: any) {
    ...
    return this.$q.all(resolve).then(function(locals: any) {

      let template = transformTemplate(locals.$$ngTemplate, options);
      let element = options.element || ng.element('<div>').html(template.trim()).contents();

      return self.compileElement(locals, element, options);
    });
  ...
  public compile(options: any) {
    if (options.contentElement) {
      return this.prepareContentElement(options);
    } else {
