(function(ng) {
     ...
     */
    let pointer, lastPointer, maxClickDistance = 6;
    let forceSkipClickHijack = false, disableAllGestures = false;
    
    /**
     * The position of the most recent click if that click was on a label element.
     * @type {{x: number, y: number}|null}
     */
    let lastLabelClickPos = null;
    ...
    function MdGesture($$MdGestureHandler, $$rAF, $timeout) {
      ...
      const touchActionProperty = getTouchAction();
      const hasJQuery =  (typeof window.jQuery !== 'undefined') && (ng.element === window.jQuery);
      ...
      function register(element, handlerName, options) {
        const handler = HANDLERS[handlerName.replace(/^\$md./, '')];
        if (!handler) {
      ...
      function addHandler(name, definition) {
        const handler = new $$MdGestureHandler(name);
        ng.extend(handler, definition);
      ...
      return self
        ...
        .handler('hold', {
          ...
          onStart: function (ev, pointer) {
            ...
            this.state.timeout = $timeout(ng.bind(this, function holdDelayFn() {
          ...
          onMove: function (ev, pointer) {
            ...
            // set in onStart
            const dx = this.state.pos.x - pointer.x;
            const dy = this.state.pos.y - pointer.y;
        ...
        .handler('drag', {
          ...
          onMove: function (ev, pointer) {
            let shouldStartDrag, shouldCancel;
    ...
    function MdGestureHandler() {
      ...
      GestureHandler.prototype = {
        ...
        start: function (ev, pointer) {
          ...
          const parentTarget = this.getNearestParent(ev.target);
          // Get the options from the nearest registered parent
          const parentTargetOptions = parentTarget && parentTarget.$mdGesture[this.name] || {};
    
          this.state = {
            isRunning: true,
            // Override the default options with the nearest registered parent's options
            options: ng.extend({}, this.options, parentTargetOptions),
    ...
    function attachToDocument( $mdGesture, $$MdGestureHandler ) {
      ...
      function mouseInputHijacker(ev) {
        const isKeyClick = !ev.clientX && !ev.clientY;
      ...
      function clickHijacker(ev) {
        const isKeyClick = ev.clientX === 0 && ev.clientY === 0;
        const isSubmitEvent = ev.target && ev.target.type === 'submit';
    ...
    function updatePointerState(ev, pointer) {
      const point = getEventPoint(ev);
      const x = pointer.x = point.pageX;
      const y = pointer.y = point.pageY;
