(function(ng) {
  ...
  registerLayoutAPI( ng.module('material.core.layout', ['ng']) );
  ...
  function registerLayoutAPI(module){
    ...
    ng.forEach(BREAKPOINTS, function(mqb) {

      // Attribute directives with expected, observable value(s)
      ng.forEach( API_WITH_VALUES, function(name){
        const fullName = mqb ? name + "-" + mqb : name;
        module.directive( directiveNormalize(fullName), attributeWithObserve(fullName));
      });

      // Attribute directives with no expected value(s)
      ng.forEach( API_NO_VALUES, function(name){
        const fullName = mqb ? name + "-" + mqb : name;
  ...
  /**
   ...
   function detectDisabledLayouts() {
     const isDisabled = !!document.querySelector('[md-layouts-disabled]');
    ...
    return [ '$timeout', function($timeout){
      return {
        ...
        compile  : function( element ) {
          if (!config.enabled) return ng.noop;
  ...
  function attributeWithObserve(className) {
    ...
    function translateWithValueToCssClass(scope, element, attrs) {
      const updateFn = updateClassWithValue(element, className, attrs);
  ...
  /**
   * Creates a registration function for ngJS Material Layout attribute directive.
  ...
  function updateClassWithValue(element, className) {
    ...
    return function updateClassFn(newValue) {
      const value = validateAttributeValue(className, newValue || "");
