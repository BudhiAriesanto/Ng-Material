...
function SidenavService($mdComponentRegistry, $mdUtil, $q, $log) {
  const errorMsg = "SideNav '{0}' is not available! Did you use md-component-id='{0}'?";
  const service = {
  ...
  return function(handle, enableWait) {
    ...
    const shouldWait = enableWait === true;
    const instance = service.find(handle, shouldWait);
 ...
 */
SidenavDirective.$inject = ['$mdMedia', '$mdUtil', '$mdConstant', '$mdTheming', '$mdInteraction', '$animate',
...
function SidenavDirective($mdMedia, $mdUtil, $mdConstant, $mdTheming, $mdInteraction, $animate,
  ...
  function postLink(scope, element, attr, sidenavCtrl) {
    ...
    let backdrop;
    let disableScrollTarget = null;
    let disableCloseEvents;
    let triggeringInteractionType;
    let triggeringElement = null;
    let previousContainerStyles;
    let promise = $q.when(true);
    const isLockedOpenParsed = $parse(attr.mdIsLockedOpen);
    ...
    function updateIsOpen(isOpen) {
      ...
      const focusEl = $mdUtil.findFocusTarget(element) || $mdUtil.findFocusTarget(element,'[md-sidenav-focus]') || element;
      const parent = element.parent();
      let restorePositioning;
    ...
    function updateContainerPositions(parent, willOpen) {
      ...
      const scrollTop = parent[0].scrollTop;
...
function SidenavController($scope, $attrs, $mdComponentRegistry, $q, $interpolate) {
  ...
  const rawId = $attrs.mdComponentId;
  const hasDataBinding = rawId && rawId.indexOf($interpolate.startSymbol()) > -1;
  const componentId = hasDataBinding ? $interpolate(rawId)($scope.$parent) : rawId;
