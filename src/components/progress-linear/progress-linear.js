...
function MdProgressLinearDirective($mdTheming, $mdUtil, $log) {
  ...
  const DISABLED_CLASS = "_md-progress-linear-disabled";
  ...
  function postLink(scope, element, attr) {
    ...
    let lastMode;
    let isDisabled = attr.hasOwnProperty('disabled');
    const toVendorCSS = $mdUtil.dom.animator.toCss;
    const bar1 = angular.element(element[0].querySelector('.md-bar1'));
    const bar2 = angular.element(element[0].querySelector('.md-bar2'));
    const container = angular.element(element[0].querySelector('.md-container'));
    ...
    function watchAttributes() {
      attr.$observe('value', function(value) {
        const percentValue = clamp(value);
    ...
    function validateMode() {
      if ( angular.isUndefined(attr.mdMode) ) {
        const hasValue = angular.isDefined(attr.value);
        const mode = hasValue ? MODE_DETERMINATE : MODE_INDETERMINATE;
        const info = "Auto-adding the missing md-mode='{0}' to the ProgressLinear element";
    ...
    function animateIndicator(target, value) {
      ...
      const to = $mdUtil.supplant("translateX({0}%) scale({1},1)", [ (value-100)/2, value/100 ]);
      const styles = toVendorCSS({ transform : to });
