 ...
 */
const inputModule = angular.module('material.components.input', [
    'ng.material.core'
  ])
...
function mdInputContainerDirective($mdTheming, $parse, $$rAF) {
  ...
  return {
    restrict: 'E',
    compile: compile,
    controller: ['$scope', '$element', '$attrs', '$animate',ContainerCtrl]
  };

  function compile(tElement) {
    // Check for both a left & right icon
    const hasLeftIcon = tElement[0].querySelector(LEFT_SELECTORS);
    const hasRightIcon = tElement[0].querySelector(RIGHT_SELECTORS);
    ...
    return function postLink(scope, element) {
      ...
      if (hasLeftIcon || hasRightIcon) {
        ...
        $$rAF(function() {
          ...
          const iconNotRemoved = element[0].querySelector('md-icon') ||
...
function inputTextareaDirective($mdUtil, $window, $mdAria, $timeout) {
  ...
  function postLink(scope, element, attr, ctrls) {
    ...
    const hasNgModel = !!ctrls[1];
    const ngModelCtrl = ctrls[1] || $mdUtil.fakeNgModel();
    const parentForm = ctrls[2];
    const isReadonly = angular.isDefined(attr.readonly);
    const mdNoAsterisk = $mdUtil.parseAttributeBoolean(attr.mdNoAsterisk);
    ...
    const errorsSpacer = angular.element('<div class="md-errors-spacer">');
    element.after(errorsSpacer);

    const placeholderText = angular.isString(attr.placeholder) ? attr.placeholder.trim() : '';
    ...
    const isErrorGetter = containerCtrl.isErrorGetter || function() {
    ...
    function setupTextarea() {
      let isAutogrowing = !attr.hasOwnProperty('mdNoAutogrow');
      ...
      const minRows = attr.hasOwnProperty('rows') ? parseInt(attr.rows) : NaN;
      const maxRows = attr.hasOwnProperty('maxRows') ? parseInt(attr.maxRows) : NaN;
      const scopeResizeListener = scope.$on('md-resize-textarea', growTextarea);
      let lineHeight = null;
      ...
      function growTextarea() {
        ...
        if (!lineHeight) {
          ...
          const originalPadding = element[0].style.padding || '';
      ...
      function getHeight() {
        ...
        const line = node.scrollHeight - offsetHeight;
      ...
      function disableAutogrow() {
        ...
        if (hasNgModel) {
          const listenerIndex = ngModelCtrl.$formatters.indexOf(formattersListener);
      ...
      function attachResizeHandle() {
        ...
        let handle = angular.element('<div class="md-resize-handle"></div>');
        let isDragging = false;
        // let dragStart = null;
        let startHeight = 0;
        let container = containerCtrl.element;
        // let dragGestureHandler = $mdGesture.register(handle, 'drag', { horizontal: false });
      ...
      if (attr.hasOwnProperty('mdDetectHidden')) {
        ...
        const handleHiddenChange = function() {
          let wasHidden = false;

          return function() {
            const isHidden = node.offsetHeight === 0;
...
function mdMaxlengthDirective($animate, $mdUtil) {
  ...
  function postLink(scope, element, attr, ctrls) {
    let maxlength = parseInt(attr.mdMaxlength);
    if (isNaN(maxlength)) maxlength = -1;
    const ngModelCtrl = ctrls[0];
    const containerCtrl = ctrls[1];
    let charCountEl, errorsSpacer;
    let ngTrim = angular.isDefined(attr.ngTrim) ? $mdUtil.parseAttributeBoolean(attr.ngTrim) : true;
    const isPasswordInput = attr.type === 'password';
    ...
    ngModelCtrl.$validators['md-maxlength'] = function(modelValue, viewValue) {
      ...
      let elementVal = element.val() || viewValue;
...
function placeholderDirective($compile) {
  ...
  function preLink(scope, element, attr, inputContainer) {
    ...
    const label = inputContainer.element.find('label');
    const noFloat = inputContainer.element.attr('md-no-float');
    ...
    if (element[0].nodeName !== 'MD-SELECT') {
      ...
      const newLabel = angular.element(
...
const visibilityDirectives = ['ngIf', 'ngShow', 'ngHide', 'ngSwitchWhen', 'ngSwitchDefault'];
...
let $$AnimateRunner, $animateCss, $mdUtil;
...
function ngMessageAnimation($$AnimateRunner, $animateCss, $mdUtil, $log) {
  ...
  return {
    enter: function(element, done) {
      const animator = showMessage(element);

      animator.start().done(done);
    },

    leave: function(element, done) {
      const animator = hideMessage(element);
...
function showInputMessages(element, done) {
  ...
  const messages = getMessagesElement(element);
...
function showMessage(element) {
  const height = parseInt(window.getComputedStyle(element[0]).height);
  const topMargin = parseInt(window.getComputedStyle(element[0]).marginTop);

  const messages = getMessagesElement(element);
  const container = getInputElement(element);

  // Check to see if the message is already visible so we can skip
  const alreadyVisible = (topMargin > -height);
...
function getInputElement(element) {
  const inputContainer = element.controller('mdInputContainer');
