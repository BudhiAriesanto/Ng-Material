...
const MD_PANEL_Z_INDEX = 80;
const MD_PANEL_HIDDEN = '_md-panel-hidden';
...
MdPanelService.prototype.create = function(preset, config) {
  ...
  if (angular.isDefined(config.id) && this._trackedPanels[config.id]) {
    const trackedPanel = this._trackedPanels[config.id];
  ...
  const panelRef = new MdPanelRef(this._config, this._$injector);
...
MdPanelService.prototype.open = function(preset, config) {
  const panelRef = this.create(preset, config);
...
MdPanelService.prototype._wrapContentElement = function(contentElement) {
  const wrapper = angular.element('<div class="md-panel-outer-wrapper">');
...
MdPanelRef.prototype.open = function() {
  ...
  return this._$q(function(resolve, reject) {
    const done = self._done(resolve, self);
    const show = self._simpleBind(self.show, self);
    const checkGroupMaxOpen = function() {
...
MdPanelRef.prototype.close = function(closeReason) {
  ...
  return this._$q(function(resolve, reject) {
    self._callInterceptors(MdPanelRef.interceptorTypes.CLOSE).then(function() {
      const done = self._done(resolve, self);
      const detach = self._simpleBind(self.detach, self);
      let onCloseSuccess = self.config['onCloseSuccess'] || angular.noop;
...
MdPanelRef.prototype.attach = function() {
  ...
  return this._$q(function(resolve, reject) {
    const done = self._done(resolve, self);
    const onDomAdded = self.config['onDomAdded'] || angular.noop;
    const addListeners = function(response) {
...
MdPanelRef.prototype.detach = function() {
  ...
  const self = this;
  const onDomRemoved = self.config['onDomRemoved'] || angular.noop;
...
MdPanelRef.prototype.show = function() {
  ...
  return this._$q(function(resolve, reject) {
    const done = self._done(resolve, self);
    const onOpenComplete = self.config['onOpenComplete'] || angular.noop;
    const addToGroupOpen = function() {
      if (self.config.groupName) {
        angular.forEach(self.config.groupName, function(group) {
...
MdPanelRef.prototype.hide = function() {
  ...
  return this._$q(function(resolve, reject) {
    const done = self._done(resolve, self);
    const onRemoving = self.config['onRemoving'] || angular.noop;
    const hidePanel = function() {
      self.panelContainer.addClass(MD_PANEL_HIDDEN);
    };
    const removeFromGroupOpen = function() {
      if (self.config.groupName) {
        let group, index;
        angular.forEach(self.config.groupName, function(group) {
    ...
    const focusOnOrigin = function() {
      const origin = self.config['origin'];
      if (origin) {
...
MdPanelRef.prototype._compile = function() {
  ...
  return self._$mdCompiler.compile(self.config).then(function(compileData) {
    ...
    if (config.contentElement) {
      const panelEl = compileData.element;
...
MdPanelRef.prototype._addStyles = function() {
  ...
  return this._$q(function(resolve) {
    ...
    const positionConfig = self.config['position'];
    if (!positionConfig) {
      hideAndResolve();
      return; // Don't setup positioning.
    }
...
MdPanelRef.prototype._updatePosition = function(init) {
  const positionConfig = this.config['position'];
...
MdPanelRef.prototype._focusOnOpen = function() {
  if (this.config['focusOnOpen']) {
    ...
    this._$rootScope['$$postDigest'](function() {
      const target = self._$mdUtil.findFocusTarget(self.panelEl) ||
          self.panelEl;
      target.focus();
    });
  }
...
MdPanelRef.prototype._createBackdrop = function() {
  if (this.config.hasBackdrop) {
    if (!this._backdropRef) {
      const backdropAnimation = this._$mdPanel.newPanelAnimation()
...
MdPanelRef.prototype._configureEscapeToClose = function() {
  if (this.config['escapeToClose']) {
    const parentTarget = getElement(this.config['attachTo']);
    const self = this;

    const keyHandlerFn = function(ev) {
...
MdPanelRef.prototype._configureClickOutsideToClose = function() {
  if (this.config['clickOutsideToClose']) {
    const target = this.config['propagateContainerEvents'] ?
...
MdPanelRef.prototype._configureScrollListener = function() {
  ...
  if (!this.config['disableParentScroll']) {
    const updatePosition = angular.bind(this, this._updatePosition);
    const debouncedUpdatePosition = this._$$rAF.throttle(updatePosition);
    const self = this;
...
MdPanelRef.prototype.registerInterceptor = function(type, callback) {
  ...
  const interceptors = this._interceptors[type] = this._interceptors[type] || [];
...
MdPanelRef.prototype.removeInterceptor = function(type, callback) {
  const index = this._interceptors[type] ?
...
MdPanelRef.prototype._callInterceptors = function(type) {
  const self = this;
  const $q = self._$q;
  const interceptors = self._interceptors && self._interceptors[type] || [];

  return interceptors.reduceRight(function(promise, interceptor) {
    const isPromiseLike = interceptor && angular.isFunction(interceptor.then);
    let response = isPromiseLike ? interceptor : null;
...
MdPanelRef.prototype.addToGroup = function(groupName) {
  ...
  const group = this._$mdPanel._groups[groupName];
  const index = group.panels.indexOf(this);
...
MdPanelPosition.prototype._setPosition = function(position, value) {
  if (position === MdPanelPosition.absPosition.RIGHT ||
    ...
    const positions = Object.keys(MdPanelPosition.absPosition).join()
        .toLowerCase();
...
MdPanelPosition.prototype.start = function(start) {
  const position = this._isRTL ? MdPanelPosition.absPosition.RIGHT : MdPanelPosition.absPosition.LEFT;
...
MdPanelPosition.prototype.end = function(end) {
  const position = this._isRTL ? MdPanelPosition.absPosition.LEFT : MdPanelPosition.absPosition.RIGHT;
...
MdPanelPosition.prototype._validateYPosition = function(yPosition) {
  ...
  const positionKeys = Object.keys(MdPanelPosition.yPosition);
  const positionValues = [];
  for (var key, i = 0; key = positionKeys[i]; i++) {
    const position = MdPanelPosition.yPosition[key];
...
MdPanelPosition.prototype._validateXPosition = function(xPosition) {
  ...
  const positionKeys = Object.keys(MdPanelPosition.xPosition);
  const positionValues = [];
  for (var key, i = 0; key = positionKeys[i]; i++) {
    const position = MdPanelPosition.xPosition[key];
    positionValues.push(position);
...
MdPanelPosition.prototype.getTransform = function() {
  const translateX = this._reduceTranslateValues('translateX', this._translateX);
  const translateY = this._reduceTranslateValues('translateY', this._translateY);
...
MdPanelPosition.prototype._isOnscreen = function(panelEl) {
  ...
  let left = parseInt(this.getLeft());
  let top = parseInt(this.getTop());

  if (this._translateX.length || this._translateY.length) {
    const prefixedTransform = this._$mdConstant.CSS().TRANSFORM;
    const offsets = getComputedTranslations(panelEl, prefixedTransform);
    left += offsets.x;
    top += offsets.y;
  }

  const right = left + panelEl[0].offsetWidth;
  const bottom = top + panelEl[0].offsetHeight;
...
MdPanelPosition.prototype._reduceTranslateValues =
    function(translateFn, values) {
      return values.map(function(translation) {
        const translationValue = angular.isFunction(translation) ?
...
MdPanelPosition.prototype._constrainToViewport = function(panelEl) {
  const margin = MdPanelPosition.viewportMargin;
  const initialTop = this._top;
  const initialLeft = this._left;

  if (this.getTop()) {
    const top = parseInt(this.getTop());
    const bottom = panelEl[0].offsetHeight + top;
    const viewportHeight = this._$window.innerHeight;
  ...
  if (this.getLeft()) {
    const left = parseInt(this.getLeft());
    const right = panelEl[0].offsetWidth + left;
    const viewportWidth = this._$window.innerWidth;
...
MdPanelPosition.prototype._calculatePanelPosition = function(panelEl, position) {
  ...
  const panelBounds = panelEl[0].getBoundingClientRect();
  const panelWidth = Math.max(panelBounds.width, panelEl[0].clientWidth);
  const panelHeight = Math.max(panelBounds.height, panelEl[0].clientHeight);

  const targetBounds = this._relativeToEl[0].getBoundingClientRect();
...
MdPanelAnimation.prototype.animateOpen = function(panelEl) {
  const animator = this._$mdUtil.dom.animator;

  this._fixBounds(panelEl);
  let animationOptions = {};

  // Include the panel transformations when calculating the animations.
  const panelTransform = panelEl[0].style.transform || '';
  let openFrom = animator.toTransformCss(panelTransform);
  const openTo = animator.toTransformCss(panelTransform);
...
MdPanelAnimation.prototype.animateClose = function(panelEl) {
  const animator = this._$mdUtil.dom.animator;
  let reverseAnimationOptions = {};

  // Include the panel transformations when calculating the animations.
  const panelTransform = panelEl[0].style.transform || '';

  const closeFrom = animator.toTransformCss(panelTransform);
  let closeTo = animator.toTransformCss(panelTransform);
...
MdPanelAnimation.prototype._fixBounds = function(panelEl) {
  const panelWidth = panelEl[0].offsetWidth;
  const panelHeight = panelEl[0].offsetHeight;
...
function getElement(el) {
  const queryResult = angular.isString(el) ?
...
function getComputedTranslations(el, property) {
  ...
  const transform = getComputedStyle(el[0] || el)[property];
  const openIndex = transform.indexOf('(');
  const closeIndex = transform.lastIndexOf(')');
  const output = { x: 0, y: 0 };

  if (openIndex > -1 && closeIndex > -1) {
    const parsedValues = transform
