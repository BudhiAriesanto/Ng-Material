...
function MdTooltipDirective($timeout, $window, $$rAF, $document, $interpolate,
  ...
  const ENTER_EVENTS = 'focus touchstart mouseenter';
  const LEAVE_EVENTS = 'blur touchcancel mouseleave';
  const TOOLTIP_DEFAULT_Z_INDEX = 100;
  const TOOLTIP_DEFAULT_SHOW_DELAY = 0;
  const TOOLTIP_DEFAULT_DIRECTION = 'bottom';
  ...
  function linkFunc(scope, element, attr) {
    // Set constants.
    const tooltipId = 'md-tooltip-' + $mdUtil.nextUid();
    const parent = $mdUtil.getParentWithPointerEvents(element);
    const debouncedOnResize = $$rAF.throttle(updatePosition);
    let mouseActive = false;
    let origin, position, panelPosition, panelRef, autohide, showTimeout,
    ...
    function addAriaLabel(labelText) {
      ...
      const interpolatedText = labelText || $interpolate(element.text().trim())(scope.$parent);
